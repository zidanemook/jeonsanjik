// 기초 개념(basics.js, v155): 국어 『사고의 힘 논리』 1~6장 모든 문제에 규칙별 기초 개념 상자가 붙는지,
...
assert.deepEqual(Object.keys(B.boxes).sort(),[...rules].sort(),'상자는 논리 규칙마다 하나(남는 상자 없음)');
for(const id of Object.keys(B.apply))assert.ok(logic.includes(id),'대입이 있는데 논리 문제가 아니다: '+id);
...
for(const id of logic){const e=bank[id].variants[0].explanation;push(id+' 해설',e);
...
// 5-1) 995문제 모두: 상자를 그려 절 제목이 1부터 이어지고 순서가 맞는지.
let tableSections=0;
for(const id of logic){
...
eqJ(entries.map(n=>n.dataset.basics).sort(),logicParts.map(p=>p.id).sort(),'기초 개념 보기 버튼 = 논리 파트 27개');
...
console.log('PASS basics:논리 1~6장 995문제 모두 규칙 상자(33개, 표 있는 상자 '+withTable+'개 — 표는 선택, 번호는 이어 매김) + 문제별 대입, 995문제 절 순서(먼저 알아 둘 말 → 표 → 규칙 → 비교 예문 → 이 문제에 대입), 용어 뜻·예)·한자 원뜻, ✓/✗ 비교 예문, 해설 세 문단, 카드·문항·변형 0, 한자는 한글 뒤 괄호 안에만, '+overlapChecked+'; 파트별 상태 27파트 모두 기초 개념 보기 → 상자 전부 펼침(대입 없음) → 이 파트 문제 풀기, 뒤로 = 파트별 상태');
