...
(function(root){
 ...
 function create(card,exercise,input,lesson,conceptId,assisted=false){return validate({schema:1,presentation:exercise.type,assisted:!!assisted,exerciseId:exercise.exerciseId,conceptId,title:lesson?.title||card.question.split('\n')[0].slice(0,160),subject:card.subject,question:exercise.question+(exercise.image?'\n원문 이미지: '+exercise.image:''),options:exercise.type==='choice'?exercise.choices.map((s,i)=>exercise.fixedOrder?s:(i+1)+'. '+s).join('\n'):'',submittedAnswer:exercise.type==='choice'?(exercise.fixedOrder?'':(input+1)+'. ')+exercise.choices[input]:String(input),correctAnswer:exercise.type==='choice'?(exercise.fixedOrder?'':(exercise.correctIndex+1)+'. ')+exercise.choices[exercise.correctIndex]:exercise.answers.join(' / '),explanation:exercise.explanation||card.explanation});}
