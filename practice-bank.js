...
(function(root){
 ...
 judge('it-takes',['It takes about two hours to get to the airport by shuttle.','Two hours are taken to review a single grant application.','It took the whole team a month to finish the final report.','It will take quite a while to repair the roof of this house.'],1,'걸리는 시간은 가주어 It을 주어로 삼아 It takes two hours to review a single grant application으로 쓴다. 시간을 주어로 올려 수동태로 바꾸지 않는다. 나머지 세 문장은 모두 It + take + 시간 + to부정사 구조를 지켰고, 셋째처럼 사람을 넣으면 It took the whole team a month가 된다.');
