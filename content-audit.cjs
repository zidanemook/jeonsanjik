...
function catalog(){const ctx={};vm.createContext(ctx);for(const f of ['core-review-pack.js','quiz-options.js','hanneung-data.js','hanneung-explanations.js','hanneung.js','gichul-data.js','gichul.js'])vm.runInContext(fs.readFileSync(__dirname+'/'+f,'utf8'),ctx);const cards=[...ctx.CORE_REVIEW_PACK];for(const [id,l]of Object.entries(bank))if(!cards.some(c=>c.id===id))cards.push({id,subject:'영어',question:l.variants[0].question,explanation:l.variants[0].explanation});
