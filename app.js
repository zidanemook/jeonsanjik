...
function renderParts(){
 ...
 for(const u of units){
  ...
  for(const [p,x] of rows){const item=menuItem(p.title+(x.weak?' ⚠ 약함':''),x.n+'문제 · 외움 '+x.mastered+' · 틀린 적 있음 '+x.wrong+' · 안 푼 문제 '+x.fresh,()=>openScope({subject:s,topic:'',round:'part-'+p.id}),'menu-item part-item'+(x.weak?' is-weak':''));item.dataset.part=p.id;g.append(item);}
  body.append(fold);
 }
}
// END PART CHECK
...
// v151: 문제집 문제(옳으면 O, 틀리면 고친 말 쓰기)는 문제의 마지막 줄(영어 문장)을 기준으로, 고친 문장이 여럿이면 모두(첫째 ★추천) 보여 준다.
function sentenceLine(quiz){if(!quiz.sentence)return [];const lines=quiz.question.split('\n'),br=lines.find(l=>l.includes('[')),base=br?br.replace(/[\[\]]/g,''):englishOf(lines[lines.length-1]),list=[].concat(quiz.sentence),box=elem('div',undefined,'choice-sentence');box.append(elem('strong','바르게 고친 문장'),...list.map((s,k)=>fixLine(base,s,k===0&&list.length>1)));return [box];}
...
function renderView(){
 ...
 if(view==='home')renderHome();else if(view==='subject')renderSubject();else if(view==='range')renderRange();else if(view==='progress')renderProgress();else if(view==='memorize')renderMemorize();else if(view==='parts')renderParts();else renderQuiz();
}
function renderHome(){
