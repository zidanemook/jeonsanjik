...
function renderRange(){
 ...
 }else if(s==='영어'){const g=group('문제집 진도');option(g,'Day 1 문장의 구조·동사 유형',scope('','Day 1'));option(g,'Day 2 동사의 형태·명사·일치',scope('','Day 2'));option(g,'Day 3 분사·준동사·관사와 도치',scope('','Day 3'));
...
function renderQuiz(){
 ...
 if(!card){
  ...
  if(chosen.length)root.append(btn('이 범위 전부 풀기 · '+questionCount(chosen)+'문제 · 일정과 상관없이 반복',toggleDrill));
  if(scope.subject)root.append(btn('다른 범위 고르기',()=>go('range',scope.subject)));
  return;
 }
...
function renderFeedback(root,card,quiz,lesson,label){
 ...
 const source=(CORE_REVIEW_PACK.find(c=>c.id===card.id)||card).source;if(quiz.type==='text'||source)root.append(elem('small',quiz.type==='text'?(card.id.startsWith('en-day1-')?'문제집 PART 01 문장의 구조·동사 유형 정리 기반 자체 제작 연습':card.id.startsWith('en-day2-')?'문제집 PART 02 동사의 형태, 명사, 일치 정리 기반 자체 제작 연습':card.id.startsWith('en-day3-')?'문제집 Day 3 문법 포인트 찾기 훈련 기반 자체 제작 연습':'대화 학습·수일치 정리 기반 자체 제작 연습'):source,'source-line'));
