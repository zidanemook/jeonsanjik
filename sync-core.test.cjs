...
let merged=sync.merge(state(),[a,b,c]);assert.equal(merged.history.length,3);assert.equal(merged.cards[0].interval,7,'v131: next-day right answers before the 7-day due change nothing');assert.equal(merged.history.filter(h=>h.delayedFirst).length,1);
assert.deepEqual(sync.merge(merged,[a,b,c]),merged);
merged=sync.merge(state(),[a,e('wrong','2026-09-10','wrong'),c]);assert.equal(merged.cards[0].interval,7,'wrong then right the same day = relearned → 7 days');assert.equal(merged.cards[0].streak,1);assert.equal(merged.cards[0].due,'2026-09-17');assert.equal(merged.cards[0].retryAt,undefined);
