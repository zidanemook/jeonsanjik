...
function client(){
 ...
 return{store,messages,connection,answer(id,date,result){current.history.push({id,cardId:'c',date,at:date+'T12:00:00+09:00',result,mode:'quiz'});events.get('study-progress-saved')();},login(uid){auth.currentUser=uid?{uid}:null;return authCallback(auth.currentUser);}};
}
...
(async()=>{const a=client(),b=client();try{
 ...
 online=false;a.answer('offline-a','2026-09-10','wrong');b.answer('offline-b','2026-09-10','correct');await settle();assert.equal(remote.get('owner').size,1);
